...
const Auth = {
    ...
    loginDoctor(email, password) {
        ...
        if (password !== expectedPassword) {
            return { 
                success: false, 
                message: 'Invalid password. Contact admin for password reset.' 
            };
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updateLoginFormFields() {
        ...
        if (type === 'admin') {
            ...
            phoneGroup.style.display = 'none';
            passwordLabel.textContent = 'Password';
            if (passwordHelp) passwordHelp.textContent = 'Contact admin if you forgot password';
        }
    }
        ...
        if (user) {
            ...
            userSection.innerHTML = `
                <span class="user-info" style="color: var(--text-color); font-size: 0.9rem;">
                    👤 ${user.userName} ${user.role === 'patient' ? '(Patient)' : user.role === 'doctor' ? '(Doctor)' : '(Admin)'}
                </span>
            ...
            if (navLogoutBtn) {
                navLogoutBtn.addEventListener('click', () => {
                    if (Utils.confirm('Are you sure you want to logout?')) {
                        ...
                        Utils.showToast('Logged out successfully', 'success');
                        setTimeout(() => {
                            window.location.href = user.role === 'admin' ? 'pages/login.html' : '../pages/login.html';
                        }, 1000);
                    }
                });
            }
